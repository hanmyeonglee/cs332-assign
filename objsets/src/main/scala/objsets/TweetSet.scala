...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = this.filterAcc(x => !that.contains(x), that.filter(_ => true))
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val leftMostRetweeted = try { left.mostRetweeted } catch { case _: java.util.NoSuchElementException => elem }
    val rightMostRetweeted = try { right.mostRetweeted } catch { case _: java.util.NoSuchElementException => elem }
    List(elem, leftMostRetweeted, rightMostRetweeted).maxBy(_.retweets)
  }
...
object GoogleVsApple {
  ...
  private val allTweets = TweetReader.allTweets
  private def keywordContainingTweets(keywords: List[String]): TweetSet =
    allTweets.filter(p => keywords.exists(keyword => p.text.contains(keyword)))
